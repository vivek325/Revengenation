...
export async function POST(req: NextRequest) {
  ...
  if (data.code !== cleanCode) {
    await supabaseAdmin.from("otp_codes").update({ attempts: (data.attempts ?? 0) + 1 }).eq("email", key);
  ...
  // Generate a sign-in link for the user using admin API
  ...
  return NextResponse.json({ success: true, token: linkData.properties?.hashed_token });
}
